...
def persons_create():
    ...
    new_persons = Person(body['name'], body['gender'], body['dob'], body['location'],)
    print(new_persons)
    id = persons.insert_row(new_persons)
    person = persons.search_row(id)
    person_dict = {'id':person[0], 'name':person[1], 'gender':person[2], 'dob':person[3],'location':person[7]}
...
def persons_read_all():
    ...
    for person in persons_list:
        persons_dict.append({'id':person[0], 'name':person[1], 'gender':person[2], 'dob':person[3],'location':person[7]})
...
def persons_update(id):
    ...
    old_person_obj = []
    old_person_obj.append(body['name'])
    old_person_obj.append(body['gender'])
    old_person_obj.append(body['dob'])
    old_person_obj.append(body['location'])
    old_person_obj.append(id)
    old_person_obj = tuple(old_person_obj)
    persons.update_row(old_person_obj)
